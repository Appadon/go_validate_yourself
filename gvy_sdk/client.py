...
ConfigPath = Union[str, os.PathLike[str]]
...
class Gvy:
	...
	def start(
		...
		"""start launches the server process and returns a configured client."""
		client = cls(
			host=host,
			port=port,
			binary_path=binary_path,
	...
	def load_config(self, config_path: ConfigPath) -> Dict[str, Any]:
		"""load_config reads a GVY run config JSON object from disk.

		Args:
			config_path: Path to a JSON GVY run config file.

		Returns:
			The decoded config object.

		Raises:
			GvyError: If the file cannot be read, decoded, or is not a JSON object.
		"""
		...
		try:
			...
			raise GvyError(f"failed reading config_path {path}: {error}") from error
		except json.JSONDecodeError as error:
			raise GvyError(f"failed decoding config_path {path}: {error}") from error

		if not isinstance(config, dict):
			raise GvyError("config_path must contain a JSON object")
		return config
	...
	def resolve_config(
		...
		*,
		...
		return self._request(
			"POST",
			"/api/config/resolve",
			payload=self._config_from_payload(config, config_path),
			timeout=self.settings.request_timeout,
		)
	...
	def run_validate_auto(self, **payload: Any) -> Dict[str, Any]:
		...
		response = self.run_config(self._auto_payload_to_config(request_payload))
		return self._auto_response_from_config_run(response)

	# Config files are loaded client-side because the server API accepts JSON
	# payloads, not filesystem paths. Unknown fields stay a server concern.
	def _config_from_payload(
		self,
		config: Optional[Dict[str, Any]],
		config_path: Optional[ConfigPath],
	) -> Dict[str, Any]:
		"""_config_from_payload returns one config object from a dict or path."""
		if config is not None and config_path is not None:
			raise GvyError("pass either config or config_path, not both")
		if config_path is not None:
			return self.load_config(config_path)
		if config is None:
			raise GvyError("config or config_path is required")
		if not isinstance(config, dict):
	...
	# The config endpoint returns pipeline-shaped data. This adapter keeps the
	# legacy convenience methods close to the old /run/validate-auto response.
	def _auto_response_from_config_run(self, response: Dict[str, Any]) -> Dict[str, Any]:
		"""_auto_response_from_config_run returns a legacy-compatible auto response."""
		...
		pipeline_result = response.get("result", {})
		if not isinstance(pipeline_result, dict):
			pipeline_result = {}

		legacy_result = {
			"main_input_csv": inputs.get("main_csv", ""),
			"schema_path": inputs.get("schema", ""),
			"split_primary_key": pipeline_result.get("split_primary_key", ""),
			"split_reused": pipeline_result.get("split_reused", False),
			"split_summary": pipeline_result.get("split_summary", {}),
			"validation": pipeline_result.get("validation_dir", {}),
			"batch_summary": pipeline_result.get("batch_summary", {}),
		}
		...
		return {
			...
			"mode": "auto",
			"outputs": {
				"split_output_dir": outputs.get("split_dir", ""),
				"success_dir": outputs.get("success_dir", ""),
				"error_dir": outputs.get("error_dir", ""),
				"batch_dir": batch.get("input_dir") or outputs.get("success_dir", ""),
				"batch_export_dir": outputs.get("batch_export_dir", ""),
			},
	...
	def _resolve_binary_path(self) -> Path:
		"""_resolve_binary_path resolves the configured binary path."""
	...
	def _download_binary(self) -> Path:
		...
		try:
			...
			raise GvyError(
				f"failed downloading Gvy binary from {download_url}: HTTP {error.code}"
			) from error
		except urllib.error.URLError as error:
